...
<Window name="${windowName}" width="1920" height="1080">
  ...
  <Components>\n`;
    ...
    project.components.forEach(comp => {
      ...
      ${
        comp.label
          ? `<Label x="${comp.geometry.x + comp.geometry.width / 2}" y="${comp.geometry.y + comp.geometry.height / 2}" 
...
router.post('/:projectId/package', async (req, res) => {
  try {
    ...
    if (includePerspective) {
      ...
      await fs.writeFile(path.join(perspectiveDir, 'Main.json'), JSON.stringify(view, null, 2));
    }
...
### Perspective
1. Copy JSON files from ` /
      perspective /
      ` to your Perspective project views folder
2. Import templates if included

### Vision
1. Import XML files from ` /
      vision /
